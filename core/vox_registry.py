# core/vox_registry.py
"""Registry for Vox TTS (text-to-speech / output) engines.

Mirrors the auris_registry / cortex_registry pattern.  Every TTS engine registers
here and the core Vox plugin handles the full pipeline: generation → file write →
lip-sync dispatch → interface delivery.  Engines only need to produce audio bytes.
"""
...
class VoxCapabilities(TypedDict, total=False):
    """Capability flags a Vox engine may advertise."""
...
class VoxRegistry:
    """Central registry for all Vox TTS engines."""
    ...
    def register_engine(
        ...
        name: str,
        module_path: str,
        capabilities: Optional[VoxCapabilities] = None,
        label: str = "",
    ) -> None:
        """Register a Vox engine.

        Args:
            name:         Short unique identifier (e.g. ``"http"``, ``"harmony"``).
        ...
        log_debug(f"[vox_registry] Registered engine '{name}' -> {module_path}")
    ...
    def find_engine_by_capabilities(self, required: VoxCapabilities) -> Optional[str]:
    ...
    def load_engine(self, name: str) -> Any:
        ...
        if not module_path:
            raise ValueError(f"[vox_registry] Unknown engine: '{name}'")

        try:
            module = importlib.import_module(module_path)
        except ModuleNotFoundError as exc:
            log_error(f"[vox_registry] Cannot import '{module_path}': {exc}")
            raise ValueError(f"[vox_registry] Invalid engine module: '{name}'") from exc

        if not hasattr(module, "ENGINE_CLASS"):
            raise ValueError(
                f"[vox_registry] Module '{module_path}' does not define ENGINE_CLASS."
            )

        engine_class = module.ENGINE_CLASS
        instance = engine_class()
        self._instances[name] = instance
        log_info(f"[vox_registry] Loaded engine '{name}' ({engine_class.__name__})")
        return instance
...
def register_vox_engine(
    ...
    VOX_REGISTRY.register_engine(name, module_path, capabilities, label)
